...
/**
 * 
 * @param {lon , lat} coord this is cordinate recieved from the client
 *  
 * @returns {404 401 401 200 500 }  status codes.
 * @returns { weather data }  Read More about API returns in  https://openweathermap.org/current#fields_json
 */
export async function GET(request, { params }) {
  ...
  try {
    ...
    const data = await fetch(
      ...
      method: "GET",
      next: { revalidate: 1800, tags: [`${lat.toString()}, ${lon.toString()}`] }
    }
    );
